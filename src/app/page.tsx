...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const saveUserToDB = async () => {
      setUserChecked(false); // start loading

      try {
        console.log("Calling /api/user POST...");
        ...
        console.log("Response from /api/user/checkuser:", res);
        if (!res.ok) {
      ...
      setUserChecked(true); // done checking
    };
  ...
  // Fetch data when user is checked
  useEffect(() => {
    if (userChecked) {
      fetchDashboardData();
      fetchRecentActivity();
